        ...
        if (TosoGameAPI.isAdmin(sp)) {
            if (args.length != 0) {
                for (String name : args) {
                    ...
                    if (target == null) {
                        ...
                        if (MainAPI.isHidePlayer(p)) {
                            ...
                            sp.sendMessage(MainAPI.getPrefix(MainAPI.PrefixType.SUCCESS) + p.getName() + "の姿を表示しました。");
                            continue;
                        }
                        sp.sendMessage(MainAPI.getPrefix(MainAPI.PrefixType.SECONDARY) + ChatColor.RED + ChatColor.UNDERLINE + p.getName() + ChatColor.RESET + ChatColor.GRAY + "はすでに姿が表示されています。");
                        continue;
                    }
                }
                return;
                ...
                if (MainAPI.isHidePlayer(sp)) {
                    ...
                    sp.sendMessage(MainAPI.getPrefix(MainAPI.PrefixType.SUCCESS) + ChatColor.GREEN + "あなたの姿を表示しました。");
                    return;
                }
            ...
            for (String name : args) {
                ...
                if (target == null) {
                    ...
                    if (MainAPI.isHidePlayer(p)) {
                        ...
                        bs.sendMessage(MainAPI.getPrefix(MainAPI.PrefixType.SUCCESS) + p.getName() + "の姿を表示しました。");
                        continue;
                    }
                    bs.sendMessage(MainAPI.getPrefix(MainAPI.PrefixType.SECONDARY) + ChatColor.RED + ChatColor.UNDERLINE + p.getName() + ChatColor.RESET + ChatColor.GRAY + "はすでに姿が表示されています。");
                    continue;
                }
            }
            return;
            ...
            for (String name : args) {
                ...
                if (target == null) {
                    ...
                    if (MainAPI.isHidePlayer(p)) {
                        ...
                        cs.sendMessage(MainAPI.getPrefix(MainAPI.PrefixType.SUCCESS) + p.getName() + "の姿を表示しました。");
                        continue;
                    }
                    cs.sendMessage(MainAPI.getPrefix(MainAPI.PrefixType.SECONDARY) + ChatColor.RED + ChatColor.UNDERLINE + p.getName() + ChatColor.RESET + ChatColor.GRAY + "はすでに姿が表示されています。");
                    continue;
                }
            }
            return;
