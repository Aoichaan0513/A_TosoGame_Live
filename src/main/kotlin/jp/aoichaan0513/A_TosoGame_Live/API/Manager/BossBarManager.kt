    ...
    companion object {
        ...
        fun showBar(p: Player, time: Int = 0, maxTime: Int = 1) {
            ...
            when (GameManager.gameState) {
                ...
                GameState.GAME -> {
                    bossBar.setTitle("${ChatColor.BOLD}${ChatColor.RED}ゲーム終了まで残り${ChatColor.GRAY}: ${ChatColor.DARK_RED}${ChatColor.BOLD}${ChatColor.UNDERLINE}${TimeFormat.formatJapan(time)}${if (MissionManager.isMission) "${ChatColor.RESET}${ChatColor.GRAY} / ${ChatColor.YELLOW}${ChatColor.BOLD}ミッション実施中" else ""}")
                    bossBar.progress = time.toDouble() / maxTime
